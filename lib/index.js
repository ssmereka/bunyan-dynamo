...
const DEFAULT_AWS_API_VERSION = "2012-08-10",
  ...
  DEFAULT_TABLE_NAME = "stick-logs",
  DEFAULT_HOST_NAME = "unknown",
  DEFAULT_BATCH_SIZE = 25,
  DEFAULT_SEND_INTERVAL = 5000;  // 5 seconds
...
class DynamoStream {
  ...
  setConfig(options) {
    ...
    self.config = {
      ...
      //"hostName": options.hostname || DEFAULT_HOST_NAME,
    ...
    //if (options.aws["profile"] !== undefined) {
    //  self.config.aws.profile = "default";
    //}
  ...
  write(record, encoding, cb = function (err) {if(err) {console.log(err);}}) {
    ...
    //item["event"] = createDynamoValueObject(DYNAMO_TYPE_STRING, JSON.stringify(record));
    item["event"] = createDynamoValueObject(DYNAMO_TYPE_STRING, record);
    ...
    //var timestamp = moment(record.time).utcOffset(0).format('YYYYMMDDHHmmssSSS') + '.' + process.hrtime()[1] + '-' + record.hostname;
    ...
    if (record.msg) {
      item.message = createDynamoValueObject(DYNAMO_TYPE_STRING, record.msg);
    }
  ...
  processWriteQueue(force, cb) {
    ...
    if( ! (force || self.writeBuffer.length >= self.config.batchSize)) {
      //console.log("processWriteQueue(%s/%s): Waiting for send interval or buffer to fill.", self.writeBuffer.length, self.config.batchSize);
      return cb();
    }
      ...
      if (err) {
        ...
        self.db.batchWriteItem(batchRequest, function (err, response) {
          if (err) {
            ...
            //console.log("processWriteQueue(%s/%s): Sent %s items.", self.writeBuffer.length, self.config.batchSize, numItemsSent);
            cb()
          }
        });
      }
  ...
  static createStream(options) {
    var stream = new DynamoStream(options);
    return {
